...
## 2026-07-23：方向反轉——網頁不是主介面，對話才是

以上是原始設計，**其中「網頁 app 是主介面」這個前提已經被實際使用推翻**。這一段
不改上面的文字，因為那份推理當時是誠實的；改掉它只會讓後來的人看不出哪裡出錯。

### 原始設計錯在哪

錯的不是任何一個技術決定，是**推理的起點**。

上面「背景」那一段寫的是：「因此需要一個純前端、GitHub Pages 託管的創作輔助工具」。
這句話把結論接在「純前端好部署」後面——那是從**技術**推出來的介面，不是從
**這個人怎麼工作**推出來的。純前端、無 build、隨開隨用，全都是真的；只是
「所以他會用它」這一步，從來沒有被驗證過。

### 實際發生的事

作者（小說家本人）用了一整天，建立 21 個角色、25 條關係、15 章、36 筆伏筆。
**全部在 agent 對話裡完成。** 網頁只開過兩次，兩次都製造摩擦。他的原話：

> 「我大部份還是跟你討論比較快，完全沒有去開那個網頁。」

回頭看很清楚：這個工具的每一個動作——「城主的設定補一句」「這章寫完了」
「這個伏筆回收了」「這兩個名字是同一個人」——都是**一句話講得完的事**。
一句話講得完的事，開網頁、找分頁、找那一筆、點編輯、打字、按儲存、按同步，
是把一秒的事變成一分鐘。表單贏不過句子。

### 留下來的那一件事

**關係圖。** 空間佈局是對話給不了的：十幾條關係誰跟誰糾纏，用看的三秒，用講的
三分鐘。這不是「網頁比較方便」，是**資訊的形狀本來就是二維的**，壓成一行一行的
文字必然失真。

所以它沒有被刪掉，是被搬家：變成 `mycelium graph`，產生一個自帶樣式與 cytoscape
的單一 HTML，點兩下就開，不連網、不用伺服器、不用 PAT。做成檔案而不是網站，
是因為那份檔案裡是整部還沒出版的小說——它不該住在任何會被 push 的地方。

### 資料主本也跟著換位置

原設計裡主本是**瀏覽器的 IndexedDB**，repo 只是備份。網頁收掉之後，
**repo 的 `data/*.json` 就是主本**，沒有第二份。原本那條「瀏覽器那份會過期，
下次開網頁要先按從 GitHub 匯入」的警告連同它保護的東西一起消失了——
少一個同步方向，就少一種互相蓋掉的方式。

### GitHub Pages 沒有關掉

原本要一併關掉。後來留著，只發布一件事：`effects/` 敘事效果庫的示範頁。
理由是那是一個可重用的 MIT 函式庫，而**捲動手感這種東西，用文字描述等於沒有描述**，
要評估它就得真的捲一次。示範頁裡沒有任何劇情，用的是通用示範角色。

### 留給未來的一句話

會被推翻的通常不是實作，是**沒有被說出口的前提**。這份文件從頭到尾在論證
「怎麼做一個網頁工具」，卻沒有一行在問「他會打開它嗎」。下一次先問那一句。
