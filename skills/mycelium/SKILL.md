---
name: mycelium
description: 在終端機幫忙寫長篇小說：讀作品的設定庫/關係/伏筆/大綱回答一致性問題、直接改設定（角色、章節狀態、伏筆、關係都能改/加/刪）、把新章節抽成設定候選（含別名合併判斷）、帶著全部設定討論劇情與反轉、批量匯入舊稿、把人物關係匯出成可離線開的關係圖 HTML、產出可離線開的「設定審閱頁」（全部可瀏覽＋標出上次以來的新改動＋作者/製作/公開三種模式）。當使用者提到自己的小說/作品設定、問「某角色目前設定是什麼」「哪些伏筆還沒回收」「這段跟前面有沒有矛盾」、說「幫我把某角色改成…」「這章寫完了」「這個伏筆回收了」、丟一章要抽設定、說「關係我理不清」「畫張關係圖」、說「你生太快我看不完」「我要 review 你改了什麼」「給畫師看的設定」，或要把散在別處的設定大綱匯進來時使用。改東西一律先自動快照，隨時可還原。
---
...
## 寫一章的六個階段（這是這個 skill 的骨架）

長篇會壞掉的方式只有兩種，而且是同一件事的兩面：**該考慮的沒考慮**，
跟**已經決定的又拿出來重想**。兩種都來自「想到哪寫到哪」，所以流程要分段，
而且每一段要有一個**會擋人的東西**——光寫在文件裡的規矩，你會忘記讀。

| 階段 | 做什麼 | 擋門 |
|---|---|---|
| ① 討論 | 提選項、講取捨。**不要在這裡順手決定** | 提之前先 `decisions <關鍵字>`，確認不是已經拍板過的 |
| ② 拍板 | 使用者選了，才寫進設定 markdown | 寫的時候**帶上標記**：`定案`／`未定`／`紅線`，還要記日期與是誰提的 |
| ③ 切規格 | 把這一章切成 beat：幾個段落、各自要達成什麼 | beat 先給使用者看過再寫。切不出來＝設定還有洞，回 ① |
| ④ 動筆前 | `precheck <章節>`，把必讀／已定案／未決／紅線攤開 | **必讀的段落真的讀完再動筆。未決的先問。已定案的不要重開** |
| ⑤ 寫 | 寫正文 | —— |
| ⑥ 驗收 | 機器先跑，人再逐句讀 | 作品自己的檢查腳本要**跑不過就 exit 1**；讀出來的問題要回寫成 ② 的規則 |

三件實際發生過、這張表就是為了擋的事：

- 連續五章沒開過完整設定集，重新推導出裡面早就寫好的東西 → ④
- 使用者已經拍板的事被重新考慮，還被覆蓋成別的版本 → ①②
- 同一類錯誤犯了三次（數字宣稱），因為沒有人把它變成檢查 → ⑥

**⑥ 最容易被跳過，但它是唯一會讓前面五階持續變好的一階。** 每次逐句讀出來的問題，
問一句「這是一次性的失誤，還是一條規則」；是規則就當場寫回設定 markdown，
帶上標記，下一次 ④ 就會自動把它擋在你面前。能寫成程式判斷的（禁用詞、數字、
時間線）就寫進作品自己的檢查腳本，不要靠記性。
...
| **`mycelium decisions`** | **全書決定總帳**：拍板過的、還沒決定的、紅線，全部攤成一張表 |
...
- **已定案**：帶著「定案／已定／寫死／已拍板」的行——**不要重新考慮，也不要覆蓋**
- **未決**：帶著「未定／待定／待建／還沒拍板／留白」的行
...
不要自己決定；「已定案」的不要重開。這三件事就是這個指令存在的全部理由。
...
繼續蓋。所以每一批比較大的寫入之後，主動說一句「要不要產一份審閱頁看一下」。
...
名字是不是「已經存在」，比對前會先正規化（全形半形、頭尾與中間的空白、英文大小寫
都不算差異），所以模型多打一個空白不會生出分身。`validate` 與 `propose` 會先跟現有
設定庫對過，把「這個是新的／這個已經有了」印在人勾選之前。

只有一種情況會停下來要人處理：候選說「A 是 B 的別名」，但 A 在設定庫裡本來就是
**獨立的一筆**。把兩筆既有紀錄併成一筆得決定誰的設定、關係、伏筆留下來，那是會蓋掉
既有資料的動作，不自動做——log 會告訴你該怎麼手動合併。
...
node scripts/mycelium.mjs review --mode 作者             # 只產作者那一份
node scripts/mycelium.mjs review --mode 公開 --volume 2  # 公開到第 2 卷
node scripts/mycelium.mjs review --out ~/桌面/審閱 --history 80
```

產出是**自帶樣式與 cytoscape 的單一 HTML**，點兩下就開。它做三件事：

1. **全部可瀏覽**：角色、關係、章節、伏筆。沒有任何關係的角色在關係圖上看不到，
   但在這裡看得到——這正是它存在的理由之一。
2. **標出上次以來的新改動**：每一次寫入都是作品 repo 的一個 commit，指令會讀
   git 歷史，算出「哪一筆是什麼時候被誰動的」。頁面記住上次檢視時間
   （localStorage），下次打開只剩沒看過的；有「標記全部已讀」。
3. **三種模式**：

| 模式 | 看得到 | 檔名 |
|---|---|---|
| 作者 | 全部：表層、底層、伏筆、改動標記 | `<作品>.author.review.html` |
| 製作 | 表層人格、視覺設定、生成提示詞、關係、章節 | `<作品>.production.review.html` |
| 公開 | 依卷數解鎖、且已整理過表層的條目 | `<作品>.public-v<N>.review.html` |

**三種模式是三個檔案，不是同一個檔案切換顯示。** 製作與公開那兩份裡，底層內容
連 DOM 都沒有——過濾是在產生檔案的時候做完的。這一點不可以改成前端隱藏：
看一次原始碼就破功。頁面上方可以互相切換（就是連到旁邊那個檔案）。

分層靠的是設定內文本來就有的標題：

```
════ 【表層｜寫的時候看這裡】 ════   ← 只有標題含「表層」的段落會被留下來
════ 【底層｜寫的時候朝這裡】 ════   ← 其餘一律當底層
```

規則是**白名單**：沒有標記的整段文字一律當底層。所以要讓某段設定給畫師看到，
就在那段前面加一行 `【表層】`。伏筆整個 store 在非作者模式不進檔案。

⚠ 跟 `graph` 同一套保護：預設寫在本機快取，**拒絕**寫進有 remote 的 git 工作目錄，
本 repo 的 `.gitignore` 另外擋掉 `*.review.html`。
...
這些欄位目前**全是空的**——等使用者真的要開始發稿畫圖時再填，不要自己編。
